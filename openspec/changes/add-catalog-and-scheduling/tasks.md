...
- [ ] 1.1 Migración `catalog_items` (esquema de design.md D4: kind,
...
- [ ] 1.2 Reglas de negocio en `Domain/Catalog`: agendabilidad
  (`isBookable`) y validación de vínculo de valoración (mismo tenant, sin
  cadenas). Tests unitarios.
- [ ] 1.3 Presentación de precio en `Domain/Catalog`: factory de
...
- [ ] 2.1 `AdminCatalogItemController` + FormRequest con validación de
  ...
  viewer.
- [ ] 2.2 Exponer catálogo en `AdminPanelDataBuilder` (lista para la vista
  admin) y mensajes i18n `en`/`es_CO` de validación.

## 3. Google Calendar — conector

- [ ] 3.1 Interfaz `GoogleCalendarClient` (`freeBusy`, `createEvent`,
...
- [ ] 3.2 Implementación HTTP real con `Http::` de Laravel: refresco de
...
- [ ] 3.3 Flujo OAuth: endpoint admin (gate
  ...
  403 para viewer.
- [ ] 3.4 Estado de conexión por línea (`connected|broken|none`) expuesto
  en `AdminPanelDataBuilder`; marcado de `broken` al detectar
  `invalid_grant`.

## 4. Tools de agendamiento

- [ ] 4.0 Intent `service_scheduling` en `AgentPackRegistry`
...
- [ ] 4.1 Tool `get_service_details`: resolución por id tenant-scoped,
  retorno de detalle con política de valoración, registro de eventos.
  Registro en `AppServiceProvider` (tag `agent-tools`). Tests.
- [ ] 4.2 Cómputo de slots server-side (`Domain/Scheduling` o similar):
  ...
  cerrado, timezone).
- [ ] 4.3 Tool `check_availability`: usa el cómputo de slots; para ítems
  con valoración usa la duración del ítem vinculado y lo señala; handoff
  cuando no hay conexión. Tests.
- [ ] 4.4 Tool `create_appointment`: re-validación del slot antes de
...
- [ ] 4.5 Índice de catálogo en `PromptBuilder` vía `AgentContextLoader`
  (ids, precio legible, duración, requisito de valoración; omitido sin
  catálogo). Test de integración del prompt.

## 5. Frontend admin

- [ ] 5.1 Módulo de catálogo: tipos + llamadas API en
...
- [ ] 5.2 Conexión de calendario en `LinesView` (NO en CredentialsView,
  ...
  `broken`, i18n.
- [ ] 5.3 Copy de tools nuevas en `toolCopyKeys` (ToolsView) + i18n, y
  etiqueta `google_calendar` en los selects/labels de provider de
  credenciales (admin y platform CredentialsView).
- [ ] 5.4 `npm run typecheck` y build del frontend en verde.

## 6. Validación final

- [ ] 6.1 Suite backend completa (`php artisan test`) en verde; revisar
